...
class Course extends CoreModel
{
    ...
    public function save()
    {
        ...
        // vu que notre programeContent est un tableau, il faut le transformer
        // en chaine de caractères pour le stocker en BDD
        // on a aussi besoin de pouvoir le retransformer en tableau quand on le récupèrera depuis la BDD
        // on l'encode donc en JSON pour pouvoir le décoder à la récupération et recréer un tableau
        $programContent = json_encode($this->getProgramContent());


        // Nous allons utiliser une requete prépaprée
        // qui permet via PDO de sécurisé insertion des données (échapment, etc)
        ...
        // les clés seront substituées par les valeurs lors de l'exécution
        $dataSql = [];
        $dataSql[":title"] = $this->getTitle();
        $dataSql[":image"] = $this->getImage();
        $dataSql[":shortDescription"] = $this->getShortDescription();
        $dataSql[":description"] = $this->getDescription();
        $dataSql[":programContent"] = $programContent;
        $dataSql[":numberOfHours"] = $this->getNumberOfHours();
        $dataSql[":price"] = $this->getPrice();
        $dataSql[":classDate"] = $this->getClassDate();
        $dataSql[":professor"] = $this->getProfessor();
        $dataSql[":modality"] = $this->getModality();
        $dataSql[":requiredLevel"] = $this->getRequiredLevel();

        // exécution de la requête
        $isSave = $pdoStatement->execute($dataSql);
